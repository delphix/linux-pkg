...
* **DELPHIX_PACKAGE_MIRROR_MAIN, DELPHIX_PACKAGE_MIRROR_SECONDARY**: When
  ...
  DELPHIX_PACKAGE_MIRROR_SECONDARY also accepts the literal value `none`, which
  leaves the secondary mirror out of the apt configuration entirely. The
  secondary mirror carries the PPAs and is Delphix's own, so it has no upstream
  equivalent, and naming one is impossible when building against the public
  Ubuntu archive rather than a Delphix mirror -- which is the situation at the
  start of an Ubuntu LTS upgrade, before the new suite's mirror sync has
  published anything. Without this, `apt-get update` fails on the secondary
  mirror's missing suite and no package can be built at all. Packages that
  build-depend on something only the PPAs carry will not resolve, so this is for
  manual builds against upstream Ubuntu; a real build wants the pinned snapshot.
